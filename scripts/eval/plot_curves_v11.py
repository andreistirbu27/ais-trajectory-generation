#!/usr/bin/env python3
"""
plot_curves_v11.py — Plot training curves for v11 (pointer over candidates).

v11 metrics.csv columns:
    global_step, epoch, train_loss, val_loss,
    val_ce, val_offset, val_smooth, val_top1

Mid-step rows have train_loss + val_loss (~approx); epoch-end rows also have
val_ce / val_offset / val_smooth / val_top1.

Usage:
    python3 scripts/eval/plot_curves_v11.py --csv runs/trajgen_v11_lite/metrics.csv
"""
...
def main():
    ...
    p.add_argument("--csv", required=True, help="Path to metrics.csv from train_gen_v11.py")
    ...
    if not epoch_df.empty:
        ...
        ax2b.set_ylabel("Top-1 accuracy")
        ax2b.set_ylim(0, 1)
    ...
    fig.suptitle(f"v11 Pointer Training  ({total_steps:,} steps | {n_epochs} epochs)",
                 fontsize=12)
    plt.tight_layout()
